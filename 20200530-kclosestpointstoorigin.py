...
class Solution:
    def kClosest(self, points: List[List[int]], K: int) -> List[List[int]]:
        distances = [
            (sqrt(point[0] * point[0] + point[1] * point[1]), point) for point in points
        ]
        return [pt[1] for pt in sorted(distances)[0:K]]
...
if __name__ == "__main__":
    ...
    testlib.run(
        lambda t, tc: t.assertEqual(
            sorted(Solution().kClosest(tc[0], tc[1])), sorted(tc[2]), tc
        ),
        testdata,
    )
